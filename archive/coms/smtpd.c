...
#define MAX_PATH_LINE 1000
#define PATH_LINE_FULL_LEN 2*MAX_PATH_LEN
/* Coming soon */
#define authorized 0
...
struct smtp_transaction {
    ...
    char* tx_rev_path;                  /* The reverse path (sender). */
    char* tx_fwd_path[MAX_RCPT];        /* The forward paths (recipients). */
    int tx_fwd_path_len;
...
char* save_path(char* target, struct input_iterator* it, int n)
{
    char c;
    ...
    for (; c != '\r' && c != '>'; c = input_next(it), target++) {
...
void populate_path(char* t, char* r)
{
    memcpy(t, "/tmp/", 5);
    memcpy(t + 5, r, RAND_LEN);

    memcpy(t + MAX_PATH_LEN, "/new/", 5);
    memcpy(t + MAX_PATH_LEN + 5, r, RAND_LEN);
}
...
void mail_command(struct smtp_context* sc, struct input_iterator* it, struct smtp_response* res_r)
{
    ...
    /* Save reverse path */
    sc->smtp_tx.tx_rev_path = malloc(PATH_LINE_FULL_LEN);
    t = save_path(&sc->smtp_tx.tx_rev_path, it, MAX_PATH_LEN);
    populate_path(t, sc->smtp_tx.tx_r);
...
void rcpt_command(struct smtp_context* sc, struct input_iterator* it, struct smtp_response* res_r)
{
    ...
    /* Save forward_path */
    sc->smtp_tx.tx_fwd_path[sc->smtp_tx.tx_fwd_path_len] = malloc(PATH_LINE_FULL_LEN);
    t = save_path(&sc->smtp_tx.tx_str_head, it, MAX_PATH_LEN);

    if (stat(smtp_off_to_str(sc, sc->smtp_tx.tx_fwd_path[sc->smtp_tx.tx_fwd_path_len]), &st) == 0) {

        /* Print rest of /tmp/ & /new/ path */
        populate_path(t, sc->smtp_tx.tx_r);
